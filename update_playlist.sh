...
for url in "${URLS[@]}"; do
  # Baixa, remove espaços em branco extras e remove cabeçalhos repetidos
  curl -sL "$url" | sed '/^#EXTM3U/d' >> "$TEMP_RAW"
done

echo "🧹 Removendo duplicados (Método Acumulativo)..."
# Esta lógica acumula os metadados e só imprime se a URL final for inédita
awk '
  /^#EXTINF/ || /^#EXTGRP/ || /^#\(comentário\)/ { 
...
echo "✅ Concluído!"
echo "Canais encontrados: $(grep -c "^#EXTINF" "$OUTPUT")"
